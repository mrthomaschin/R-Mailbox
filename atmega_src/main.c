...
int packageIR_Tick(int packageIR_state)
{
	...
	{
		case Detect_Package:
			isPackage = (detectPackage() || currentWeight > 5);
			break;
...
enum outgoingButton_states { outgoingButton_Idle, outgoingButton_Press, outgoingButton_Release};
...
enum flag_states { flag_Lowered, flag_Raised};
...
enum Scale_States { Scale_Init, Scale_On} Scale_State;
int scale_Tick(int Scale_State)
{
	//Holds our value raw value from the scale while we convert it
	long rawWeight = 0;
	...
	switch(Scale_State)
	{
		...
		case Scale_On:
		//Read scale 10 times
		rawWeight = getAverage(5);
		//407 is a magic number unfortunately, converts our value to grams
		rawWeight = rawWeight/407;
...
int main(void)
{
	...
	//PB3 - Servo Wire, PB4 to PB7 - SPI Connection to Pi
	DDRB = 0xFF; PORTB = 0x00;	
	//PC0 - Outgoing Mail Button, PC1 - Authorize signal from security Pi, PC6 + PC7 Scale data transfer
	...
	unsigned long scale_Tick_period = 500;
	...
	// Task 6
	task6.state = Scale_Init;
	task6.period = scale_Tick_period;
	task6.elapsedTime = scale_Tick_period;
	task6.TickFct = &scale_Tick;
